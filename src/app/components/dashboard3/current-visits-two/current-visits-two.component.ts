...
@Component({
  selector: 'app-current-visits-two',
  standalone: true,
  imports: [TablerIconsModule, MaterialModule],
  templateUrl: './current-visits-two.component.html',
})
export class AppCurrentVisitsTwoComponent implements AfterViewInit {
  constructor(
  ...
  ngAfterViewInit() {
    ...
    this.browserOnly(() => {
      let root = am5.Root.new('current-visitors-two');
